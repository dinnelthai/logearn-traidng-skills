...
executor = TradeExecutor(
    ...
    logearn_cli_path="/path/to/logearn-cli.py"
)
...
# 计算买入金额
amount = pm.calculate_position_size(
    total_capital=2.0,
    tier="buy_618"  # 3%
)
print(f"买入金额: {amount} SOL")  # 0.06 SOL
...
if action.should_sell:
    ...
    print(f"收益率: {action.profit_rate*100}%")
...
|------|--------|------|
| `wallet` | - | 钱包地址（必填） |
| `logearn_cli_path` | 自动检测 | LogEarn CLI路径 |
...
action = profit_mgr.check_profit_target(
    current_price=current_price,
    avg_price=avg_price,
    profit_50_sold=state.get("profit_50_sold", False),
    ao_active=ao_active
)
